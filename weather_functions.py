...
def make_weather_home_call():
    api_endpoint = f"https://api.openweathermap.org/data/2.5/weather?zip={home_zip}&appid={weather_api_key}&units=imperial"
    ...
    if home_weather_response.status_code == 200:
        ...
        current_temp_home = home_weather_data["main"]["temp"]
        max_temp_home = home_weather_data["main"]["temp_max"]
        home_location = home_weather_data["name"]
...
def make_weather_work_call():
    api_endpoint = f"https://api.openweathermap.org/data/2.5/weather?zip={work_zip}&appid={weather_api_key}&units=imperial"
    ...
    if work_weather_response.status_code == 200:
        ...
        current_temp_work = work_weather_data["main"]["temp"]
        max_temp_work = work_weather_data["main"]["temp_max"]
        work_location = work_weather_data["name"]
